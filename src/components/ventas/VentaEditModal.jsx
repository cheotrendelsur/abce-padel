...
export default function VentaEditModal({ venta, onClose, onUpdated }) {
  ...
  useEffect(() => {
    ...
    supabase.from('venta_items').select('*').eq('venta_id', venta.id)
      .then(({ data }) => {
        if (data && data.length) setItems(data.map(d => ({ producto_id: d.producto_id, cantidad: d.cantidad })))
        else setItems([{ producto_id: '', cantidad: 1 }])
      })
  }, [venta.id])
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      const { error: ve } = await supabase.from('ventas').update({
        nombre_cliente: nombreCliente, fecha_venta: fechaVenta,
        forma_pago: formaPago, moneda, monto_total: parseFloat(montoTotal), notas,
      ...
      // 3. Insertar nuevos items (trigger resta nuevo stock)
      const newItems = items
        .filter(it => it.producto_id && it.cantidad > 0)
        .map(it => ({ venta_id: venta.id, producto_id: it.producto_id, cantidad: parseInt(it.cantidad) }))
        ...
        <form onSubmit={handleSave} className="p-5 space-y-4">
          ...
          {montoTotal && (
            <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-2 flex justify-between">
              <span className="text-sm text-green-700 font-medium">Comisión (25%)</span>
          ...
          <div>
            ...
            <button type="button" onClick={addItem} className="mt-2 text-primary text-sm font-medium hover:underline">
              + Agregar producto
            </button>
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading}
              className="flex-1 bg-primary text-white font-semibold py-2.5 rounded-xl hover:bg-primary-dark transition disabled:opacity-60 text-sm">
